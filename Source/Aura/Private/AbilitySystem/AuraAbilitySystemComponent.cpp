...
void UAuraAbilitySystemComponent::AbilityInputTagHeld(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		...
		{
			// Указывает способности что она зажата
			AbilitySpecInputPressed(AbilitySpec);
			if (!AbilitySpec.IsActive())
			{
				// Попытка активации способности, если по какой то причине она не запустилась
...
void UAuraAbilitySystemComponent::AbilityInputTagReleased(const FGameplayTag& InputTag)
{
	...
	for (FGameplayAbilitySpec& AbilitySpec : GetActivatableAbilities())
	{
		if (AbilitySpec.DynamicAbilityTags.HasTagExact(InputTag))
		{
			// Указывает способности что кнопку отпустили
...
{
	// Блокировать Ability List до тех пор пока не завершится обход по foreach
